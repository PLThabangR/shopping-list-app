...
//get items search array
const searchResults = useSelector((state:RootState) => state.items.searchResults);
...
const items = useSelector((state:RootState) => state.items.items);
   ...
   //display item on seacrh array
   const DisplaySearchedItem= searchResults.map((item:Item) => <ItemCard key={item.id} id={item.id} email={item.email} category={item.category} name={item.name} quantity={item.quantity} notes={item.notes} imageUrl={item.imageUrl}  />)
  ...
  return (
    ...
    <Navbar/>
        ...
        <p className="text-base font-light shrink-0 text-[#3C3D42] md:text-2xl text-center  m-1 p-1">Let you shopping plan become a success</p>
          ...
          {items.length === 0 ?<h1 className="text-2xl shrink-0 font-extrabold text-[#3C3D42] md:text-5xl text-center  m-2 p-2">No items found</h1>:<h1 className="text-2xl shrink-0 font-extrabold text-[#3C3D42] md:text-5xl text-center  m-2 p-2">Avaliable items</h1>}
        <div className="flex flex-col justify-center items-center  md:flex-row md:justify-evenly md:flex-wrap gap-1 ">
             
            {searchResults.length > 0 ?DisplaySearchedItem:DisplayItems  }
